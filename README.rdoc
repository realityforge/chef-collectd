...
* collectd::client - Install collectd and configure it to send data to servers defined by `node[:collectd][:plugin][:network][:server]`.
* collectd::server - Install collectd and configure it to recieve data from clients.
* collectd::rrdtool_plugin - Output to RRD database files.
* collectd::syslog_plugin - Log errors to syslog.
* collectd::cpu_plugin - CPU usage.
* collectd::df_plugin - Free space on disks.
* collectd::disk_plugin - Disk I/O operations.
* collectd::interface_plugin - Network I/O operations.
* collectd::memory_plugin - Memory usage.
* collectd::swap_plugin - Swap file usage.
